...
Version: **v0.6.0-beta.2** (beta restructure; not production-ready). Python tooling is **uv** (workspace lockfile `uv.lock`); JS is **pnpm** (workspace lockfile `pnpm-lock.yaml`).
...
(`chrome-extension` was dropped in the v0.6.0 restructure; the Tauri `desktop` shell was re-added and shipped in v0.6.0-beta.2 — see [docs/DESKTOP.md](docs/DESKTOP.md).)
