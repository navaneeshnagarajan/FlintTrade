...
| Native adapters (Dhan/Upstox/Kotak Neo/INDmoney/Groww): identity, capabilities, order + **data** surfaces | 🟡 | Adapter and mapping code is present and mock-tested. Dhan and Upstox are connectable after live verification and emergency-planner coverage. INDmoney's fail-closed planner is locally verified but it remains coming soon pending an authoritative restart-time regular/smart-parent discriminator, a broker-atomic reduce-only close primitive, and a funded/live-market order-safety proof. Kotak Neo's pinned-SDK-grounded planner is locally verified; live login/read and order-safety proof remain. Neo has no sandbox (never offer Neo Practice). FT-MONDAY-002 locks Monday MSI native Dhan + Neo read/API smoke; product smoke is not in the tracking PR. Groww retains its documented live blockers. |
