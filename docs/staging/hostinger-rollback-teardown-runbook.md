...
`C:/Users/USER/Desktop/Trading/FlintTrade-wt-hostinger-local-staging-prep-e039`
...
```bash
cd "C:/Users/USER/Desktop/Trading/FlintTrade"
```
...
```bash
git worktree remove "C:/Users/USER/Desktop/Trading/FlintTrade-wt-hostinger-local-staging-prep-e039"
```
...
Do **not** run `git checkout` onto `main` as a side effect of cleanup unless you
already intended to switch branches in that clone. Do **not** mutate `main`
history.
...
From the surviving checkout (or from the prep worktree before removal if you
are only scrubbing artifacts):
...
Optional generated caches that rebuild on the next site gate (remove only if
you intentionally want a cold content gen):

```bash
rm -rf packages/apps/site/.source
```

Do **not** delete root `node_modules` as part of this runbook unless you are
also planning a full reinstall; dependency trees are shared monorepo state, not
Hostinger-specific staging artifacts.
...
| Document | Role |
| --- | --- |
| `docs/setup/hostinger-local-staging-prep.md` | Prep manifest: build commands, env contract, verification matrix |
| `flinttrade-design/baselines/rollback-runbook-2026-05-24.md` | Tone and caution reference for restructure rollback only |
...
| This file | Local discard + future teardown documentation |
...
**Rehearsal timestamp:** 2026-08-09T19:10:48+05:30
**Live prep HEAD during rehearsal:** `141766eb33b066fa78f28425a39ff8f4f741ab88`
**Throwaway branch (deleted):** `tmp/hostinger-rollback-rehearsal-52728`
**Throwaway path (removed):** `C:/Users/USER/Desktop/Trading/FlintTrade-wt-hostinger-rollback-rehearsal-tmp`
**Evidence log (local, not required for public clone):**
...
| Throwaway: fake `.next`/`out` → `rm -rf` clean (section 1.4) | Pass |
| Throwaway: `git worktree remove` then `git branch -D` (section 1.2) | Pass |
| Live prep worktree still registered and unchanged after rehearsal | Pass |
| Section 2 has no fenced shell and no `curl`/`ssh`/`scp`/`rsync` command lines | Pass |
| Section 2 carries `DO NOT EXECUTE` + assumption labels | Pass |
