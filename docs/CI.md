...
| `broker-sdk-freshness.yml` | daily cron; manual dispatch | 1 Linux job | Compares `brokers.lock` with official SDK repositories and registries. Kotak Neo checks current `main`, its tree, the latest stable tag/commit/tree, PyPI version and release artefact digests. Mutable upstream state never gates an unrelated PR; drift fails this scheduled job and uploads its manifest. |
