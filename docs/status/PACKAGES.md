...
|---|---|---|---|---|---|---|---|---|
| `packages/ai` | `flint-ai` | 0.5.0 | Python | LLM chat, RAG, ML signals, news sentiment, autonomous trading | 14 (incl. `flint-core`, `flint-indicators`) | 41 | 32 | working (tests UNKNOWN — run aborted) |
| `packages/automation` | `flint-automation` | 0.5.0 | Python | Cron scheduler, Telegram bot, OpenClaw agent bridge, post-market analysis | 7 | 25 | 10 | working (tests UNKNOWN) |
| `packages/backtest-engine` | `flint-backtest-engine` | 0.5.0 | Python | Event-driven simulation engine, 101 strategies, VectorBTRunner sweeps, walk-forward, Monte Carlo | 10 | 123 | 26 | working (tests UNKNOWN) |
...
| `packages/core` | `flint-core` | 0.5.0 | Python | Framework, CLI, OpenAlgo client, config, models, logger, Flask app server (90+ routes), admin, security, monitoring | 16 | 50 | 46 | working (tests UNKNOWN) |
| `packages/data` | `flint-data` | 0.5.0 | Python | Real-time tick capture, WS storage, trade logs, SEBI audit trails, order flow, P&L, tax reports | 7 (incl. `flint-core`) | 27 | 18 | working (tests UNKNOWN) |
| `packages/desktop` | `@flinttrade/desktop` (npm) / `flinttrade-desktop` (cargo) | 0.1.0 (npm) / 0.5.0 (cargo) | TypeScript + Rust (Tauri v2) | Native desktop wrapper for the terminal via Tauri v2 | 1 npm + 2 cargo | 6 | 0 | experimental (scaffold only) |
| `packages/ditto` | `flint-ditto` | 0.5.0 | Python | Multi-broker, multi-account trade orchestration, position mirroring, allocation modes | 6 | 7 | 5 | working (tests UNKNOWN) |
| `packages/engine` | `flint-engine` | 0.5.0 | Python | Strategy execution, order routing, 5-layer safety, kill switches, scheduler, action center, sandbox | 6 (incl. `flint-core`, `flint-data`) | 40 | 37 | working (tests UNKNOWN) |
| `packages/gateway` | `flint-gateway` | 0.5.0 | Python | Direct broker connections (33 brokers), adapter pattern, encrypted credentials, WS bridge | 5 | 29 | 16 | **broken on this machine** — `6 errors during collection` in gateway tests on Python 3.14.3 |
| `packages/historical` | `flint-historical` | 0.5.0 | Python | OHLCV downloader, free NSE data (OpenChart/yfinance/jugaad), DuckDB pipeline, expiry manager | 6 | 23 | 17 | working (tests UNKNOWN) |
| `packages/indicators` | `flint-indicators` | 0.5.0 | Python | Technical analysis — TA-Lib batch (150+), Numba streaming, Pine Script conversion | 3 (TA-Lib optional) | 16 | 14 | working (tests UNKNOWN) |
| `packages/integration` | `flint-integration` | 0.5.0 | Python | TradingView webhooks, ChartInk alerts, custom webhooks, visual flow builder (54 flow nodes), alerter | 5 | 12 | 9 | working (tests UNKNOWN) |
| `packages/screener` | `flint-screener` | 0.5.0 | Python | Market scanner, OI analysis, PCR, max pain, portfolio Greeks, GEX, IV smile, vol surface, straddle P&L, RRG, order flow | 7 | 56 | 40 | working (tests UNKNOWN) |
