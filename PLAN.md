...
- [ ] G10 (remainder) — wire `AlgoTagGuard` (per-(broker,exchange) algo-order count + algo_id relay) into `BrokerRouter` dispatch for `algo_tag_required` adapters. Deferred: it can only be exercised once a native adapter with `algo_tag_required=True` (Dhan/IndMoney) activates, which needs the credential-replay login step (G3). Ships with native activation.
