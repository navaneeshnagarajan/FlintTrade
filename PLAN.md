...
**FT-MONDAY-003** (`#254`) locks AI Chat live-read acceptance: when an LLM is configured, Chat may use Practice fills and native live-read feeds for analysis. Suggest stays labelled illustrative. Chat never shows green Connected without a real LLM. Measure later: profitable alphas are not a Monday ship criterion. This is the tracking lock only — no product in this change, and it does not lift the native broker HTTP freeze.
