...
> Approved spec: `.local/specs/archived/2026-03-19-flinttrade-v2-foundation-design.md`
> **Beta sprint plan: `.local/specs/archived/2026-03-19-beta-sprint.md`** (10 task groups, everything included)
> v0.3.0 design spec: `.local/specs/2026-03-25-v03-structured-calm-design.md`
...
- **External test-deps:** openalgo, openclaw — no longer git submodules; cloned locally under `.local/external/` via `scripts/setup-test-deps.sh` for development only. AlgoMirror's patterns are absorbed in-process by `packages/ditto/`; upstream no longer tracked.
...
- [x] Phase 10: Testing + Beta Release (1,600+ tests green, Glide DataGrid + indicators wired + Rust tick-engine — v0.1.0-beta tagged, v0.2.0-alpha released)
...
- [x] Aggressive live testing with Dhan Sandbox — Dashboard, Chart, Scalper, Watchlist, OptionChain verified during market hours
  - [x] Every widget: Dashboard, Scalper, Positions, Orders, Holdings, TradeBook, OrderPad
  - [x] Every widget: Chart, OptionChain, OIChart, Straddle, Depth, Greeks, Watchlist
  - [x] Every new widget: SectorMap, Calculator, MTM Monitor, Risk Panel, News Feed, Ticker, AI Advisor
  - [x] Every tool: Settings, P&L Dashboard, Strategy Builder, Trade Journal, Flow Builder, Market Intelligence, Backtest Lab
  - [x] Every route: /terminal, /setup, /invest, /learn, /lab, /automate, /ai, /settings, /welcome, /explore
- [x] Test with Kotak Neo sandbox if available — deferred (Dhan sandbox used instead)
- [x] Fix all bugs found during testing (50+ audit findings resolved)
...
- [x] All widgets render and display live data
- [x] All tools are functional (no stubs)
- [x] Setup wizard works end-to-end
- [x] /invest route has Portfolio Tracker + Holdings + Net Worth
- [x] `tsc --noEmit` — zero errors
- [x] `npm run build` — zero warnings (one pre-existing vendor-charts chunk warning)
- [x] `vitest run` — 547 tests pass
- [x] `make test` — 1,100+ Python tests pass
- [x] Update all documentation (CLAUDE.md, README.md, CONTRIBUTING.md, packages/terminal/CLAUDE.md)
- [x] Clean up git history
- [x] Tag v0.1.0-beta (tagged 2026-03-24)
- [x] Publish to GitHub
...
|---|---------|-------------|--------|
| 0 | **Live Market Signals Pipeline** | packages/ai | ✅ Done (Wave 1-9) |
| 1 | Chrome Extension | openalgo-chrome | ✅ Done (Wave 1-9) — packages/chrome-extension/ |
| 2 | Excel Integration | OpenAlgo-Excel | v0.6.0 |
| 3 | WhatsApp Alerts | wabridge | ✅ Done (Wave 1-9) — whatsapp_alerts.py + routes |
| 4 | Kotak Neo Cost Routing | — | v0.6.0 |
| 5 | Dhan Rolling Option API | — | v0.6.0 |
| 6 | Historical Expired Options | ExpiryTrack | ✅ Done (Wave 1-9) — expiry_tracker.py + routes |
| 7 | CLI Tools (Click) | CLI-Anything pattern | v0.6.0 |
| 8 | DDNS Auto-update | infra scripts | v0.6.0 |
| 9 | MCX Full Support | — | ✅ Done (Wave 1-9) — mcxLots.ts + 46 tests |
| 10 | QuestDB Tick Aggregation | openquest | v0.6.0 |
| 11 | Mutual Fund Explorer | jugaad-data | ✅ Done (Wave 1-9) — mf_routes.py + MutualFundTab.tsx |
| 12 | SIP Calculator | — | ✅ Done (v0.1.0-alpha) |
| 13 | ETF Tracker | etftracker | ✅ Done (wired to OpenAlgo quotes) |
| 14 | Stock Screener | openscreener | ✅ Done (wired to StockCache) |
| 15 | IPO Tracker | — | ✅ Done (Wave 1-9) — ipo_routes.py + IpoTab.tsx |
| 16 | /learn route (full) | — | ✅ Done (v0.1.0-alpha) |
| 17 | OpenClaw bridge | — | ✅ Done (Wave 1-9) — openclaw_bridge.py (ai + automation) |
| 18 | Desktop App (Tauri) | fastscalper-tauri | ✅ Done (Wave 1-9) — packages/desktop/ scaffold |
...
| 26 | Multi-agent AI (TradingAgents) | TradingAgents | ✅ Done (Wave 10-20) — multi_agent.py |
| 27 | Unsloth QLoRA Fine-tuning | — | v0.7.0 |
| 28 | Fundamental Screener | Screener.in | ✅ Done (Wave 10-20) — fundamental_screener.py |
| 29 | FII/DII Tracker | NSE | ✅ Done (Wave 10-20) — fii_dii.py |
| 30 | RRG Calculator | — | ✅ Done (Wave 10-20) — rrg.py + useRRGData.ts |
| 31 | Risk Debate + Ensemble Selector | — | ✅ Done (Wave 10-20) — risk_debate.py + ensemble_selector.py |
| 32 | Hyperopt Strategy Optimiser | — | ✅ Done (Wave 10-20) — hyperopt_strategy.py |
| 33 | Bracket Orders | — | ✅ Done (Wave 10-20) — bracket_order.py |
| 34 | Order Flow Inference | — | ✅ Done (Wave 10-20) — orderflow_inference.py |
| 35 | Activity Log (SEBI Audit) | — | ✅ Done (Wave 10-20) — activity_log.py |
| 36 | Docker Production Config | — | ✅ Done (Wave 10-20) — multi-stage, uv, tini |
...
- [x] External test-deps registry: openalgo, openclaw (cloned to `.local/external/` via `scripts/setup-test-deps.sh`; previously git submodules under `infra/`). AlgoMirror dropped from registry — its patterns are absorbed in-process by `packages/ditto/`.
...
| Repo feature map | `.local/reference/REPO_FEATURE_MAP.md` |
| Best-in-class verdicts | `.local/archive/research/BEST_IN_CLASS_2026.md` |
| Enhancement patterns | `.local/archive/blueprints/ENHANCEMENT_BLUEPRINT.md` |
| All 222 repos | `.local/archive/reference/REPOS.md` |
