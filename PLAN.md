...
- [ ] **Portfolio optimiser — BUILD** (heaviest remaining): `/v1/portfolio/optimise` + `/v1/portfolio/frontier` are real numpy/scipy Markowitz (`portfolio_optimiser.py`) with NO widget. Needs a multi-symbol basket: fetch daily history per ticker (`getHistory` ×N), build `{ticker: returns[]}` (equal-length — align/truncate to the shortest, else `pd.DataFrame(raw)` misaligns), POST, render weights + metrics (+ optional efficient-frontier scatter). Medium-large; the multi-symbol alignment is the risk.
- [ ] **Backtest walk-forward — BUILD (low priority, limited route)**: `/v1/backtest/walkforward` only walk-forwards a **built-in buy-and-hold baseline** (custom strategies are programmatic-only per its docstring), and the Lab already surfaces strategy robustness via `RobustnessCard`. So a UI for this route shows buy-and-hold robustness only — modest value. Wire as a small "baseline robustness" readout in the Lab if desired, or leave; `/backtest/compare` (`strategy_comparison_bp`) is registered but has no client.
- **LEAVE (honest sample/stub backends — widgets already badged sample; wiring would be sample→sample):** scanner (`/v1/scanner/*` — also has a latent live bug: `_registry_data_fetcher` calls `registry.get_history(symbol=…, …)` but the signature is `get_history(account_id, params)` → TypeError on live use; the route is unreached today, fix when the scanner widget is wired), economic-calendar, earnings, breadth-history.
