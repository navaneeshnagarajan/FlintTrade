...
- [ ] **OI analytics — BUILD**: `/v1/oi/analysis` (LB/SC/SB/LU OI-action signals) + `/v1/oi/unusual` (z-score OI spikes) are real + tested (`oi_analytics.py`) with NO widget. Add an "OI Signals / Unusual OI" panel (or a tab on `OIHeatmapWidget`) POSTing both, honouring `is_sample_data`. (`/v1/oi/heatmap` is real but the existing OIHeatmap recomputes the same client-side — consolidate or repoint, see §5.)
- [ ] **Portfolio optimiser — BUILD**: `/v1/portfolio/optimise` + `/v1/portfolio/frontier` are real numpy/scipy Markowitz (`portfolio_optimiser.py`) with NO widget. Build a portfolio-optimiser panel (weights + efficient frontier).
- [ ] **Backtest walk-forward / compare — BUILD**: `/v1/backtest/walkforward` (`WalkForwardAnalyser`) + `/backtest/compare` are real with no UI; extend the Lab/Backtest surface with a walk-forward mode + a strategy-compare view.
