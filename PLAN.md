...
- [x] Confirmed-dead cleanup: `portfolio_optimizer` twin, 4 screener `_analytics` twins, duplicate `whatsapp_alerter`/`openclaw_bridge`, orphan `scanner_service`, duplicate `/api/v1/errors` + `/api/v1/health`, stale `.gitignore`/`.dockerignore` paths, `flint.toml [safety]` overscope flags, `packages/ditto/` debris.
...
- [x] Restore terminal typecheck + build (shadcn primitives imported individual `@radix-ui/react-*` packages never declared in package.json → migrated to the unified `radix-ui`; 87 TS errors → 0; `vite build` green). Note: `test.yml`'s `node-core-tests` job *does* gate `tsc --noEmit` + `run build`, so this would have failed CI when Actions was unavailable on the account. No CI change needed.
- [x] Native broker adapters built to FULL doc-grounded parity (see §2): `brokers/{dhan,upstox,kotakneo,indmoney}.py` + mappings (2026-06-12 wave — no longer skeletons), capability-metadata ranking engine + `GET /api/v1/broker/recommendations`.
...
- [x] **Multi-broker capability recommender:** `recommendations.py` filters broker capabilities per use case from declared `Capabilities`; `GET /api/v1/broker/recommendations`. (Honest capability metadata corrected in the 2026-06-12 pass.)
