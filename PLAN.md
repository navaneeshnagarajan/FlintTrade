...
- [ ] **G40 — Connect-path + whole-repo consolidation (no parallel paths).** Merge the two broker-connect stacks (gateway `/v1/*` + `BROKER_CATALOG` vs native `/api/v1/native/*` + `NATIVE_AUTH_METHODS`) into ONE surface: one catalogue with a `connectable` flag (Dhan+Upstox only; every other broker "coming soon"), one OAuth, one client, one reusable `<BrokerConnect>` UI shared by Settings **and** the setup wizard. OpenAlgo primary; native secondary behind a "not fully tested — use at your own risk" warning; all UI-based (no `.env`). Spec: `.local/specs/connect-consolidation/DESIGN_LOG.md`. The whole-repo sweep is DONE (2026-07-04): 73 findings → **21 consolidation units** in `.local/specs/consolidation/BACKLOG.md` (spec of record) — incl. 2 safety bugs (U1 ungated ditto mirror order path; U2 webhook order path never gates), dead parallel broker stack (U3/U4), ditto's third credential store (U5), catalogue×3/×5 duplication (U6/U7), 5 frontend HTTP clients (U8), dual option-analytics backends (U9), 2 sandbox engines (U11), 2 latency stacks (U12), backtest/AI/screener islands (U13–U15), scheduler×3+ (U16), env-only secrets with no UI path (U18–U21). Execution: U1/U2 first, then G40 connect merge (+U3–U7), then tiers. Order gate + G9 operator-session guard + encrypted vault stay untouched. **Consolidation rule (maintainer, 2026-07-04): MERGE the union of capabilities into one implementation — never just delete a duplicate, because it may hold a feature the survivor lacks.** Landed so far (safety/correctness only, no deletions): the ditto ungated mirror order path is removed (a pure bypass — the gated path is a strict superset) + a URL-string tripwire closes the G12 grep-guard blind spot; the webhook place/cancel handlers now fail honestly instead of faking `queued`. The dead-looking modules (`webhook_server` dynamic-registry, `webhook_keys` per-webhook AES-GCM-SIV secrets, `webhook_replay` nonce defence, `broker_interface`) each hold a real capability the live path lacks and are PRESERVED — they get folded into the single receiver/registry during the relevant merge (webhooks → G23/G24), not dropped.
