...
- [x] **Portfolio optimiser — BUILT** (PortfolioOptimiserWidget, 90th widget): `/v1/portfolio/optimise` now has a UI — method selector (Markowitz/Max-Sharpe/Min-Variance/Risk-Parity/Equal-Weight), ~1y multi-symbol basket fetch with return series aligned to the common length (the misalignment risk handled), weights + return/vol/Sharpe/diversification, sample fallback. (`/v1/portfolio/frontier` not yet surfaced — an efficient-frontier scatter is a future enhancement.)
- **Backtest walk-forward — LEAVE (principled, not built)**: `/v1/backtest/walkforward` only walk-forwards a **built-in buy-and-hold baseline** (custom strategies are programmatic-only per its docstring). A "walk-forward" UI that silently tests only buy-and-hold — not the user's strategy — would MISLEAD (same line drawn on the credential-rotation scaffold: don't ship misleading functionality), and the Lab already surfaces real strategy robustness via `RobustnessCard`. Left unwired by design. `/backtest/compare` (`strategy_comparison_bp`) likewise has no client; surface only if a clear multi-strategy compare view is wanted.
- **LEAVE (honest sample/stub backends — widgets already badged sample; wiring would be sample→sample):** scanner (`/v1/scanner/*` — its latent live bug is now FIXED: `_registry_data_fetcher` calls `get_history(account_id, params)` with ISO start/end dates, resolves the primary account at build time with a sample fallback, and normalises dict/array candle shapes; live-registry tests pin the signature. The route still has no frontend caller — widget wiring remains a separate decision), economic-calendar, earnings, breadth-history.
