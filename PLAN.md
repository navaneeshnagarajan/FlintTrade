...
- [ ] Review: PIN-alone → Live-unlock policy (single-operator posture may be fine, but make it an explicit, documented decision; consider PIN+recent-login freshness).
- [ ] G10 — **Per-second order rate limiting on the reachable order path** (SEBI-derived — this phase is titled SEBI-correct, so it lands here, verified again under load in Phase 4): apply the token bucket to core `/orders/*`, smart-route, and agent routes; wire `AlgoTagGuard` (built + tested + unwired) into `BrokerRouter` dispatch for `algo_tag_required` adapters; correct the app.py:1057-1063 coverage comment (true for bracket/strategy-start, false for core orders + webhooks).

**Broker auth (the core build)**
- [ ] G3 — **Credential-replay login step** (referenced in app.py comments, does not exist): on boot + on demand, `CredentialStore.retrieve_for(adapter, account)` → native `adapter.login()` → `registry.put_session`; per-broker failure surfaces; daily re-run at token-expiry boundaries. Without this natives can never activate.
- [ ] G4 — Native credential capture UX + API: per-broker forms (Dhan access-token or PIN+TOTP mint; Upstox OAuth with real configurable `redirect_uri` + populated `oauth_url_template`s — today `oauth_start` returns a blank URL; Kotak 5-field TOTP/MPIN; IndMoney `access_token`); store under correct `adapter_id`/key names; add `kotakneo` to the catalogue (or alias `kotak`).
- [ ] G5 — Real daily session refresh: wire `CredentialsRotator` to adapter primitives (Dhan `generate_token`/`renew_token` first — the only attestable native), mount `rotation_routes`, Account Manager UI with token-expiry countdown + morning re-auth prompt (Upstox ~03:30 IST, others ~daily).
- [ ] G7 — Bridge reconnect realism: stop persisting single-use OAuth `code`/stale TOTP as replayable; store per-flow reusable material; surface "needs fresh login" instead of failing silently at boot.
...
- [ ] **Exit:** fresh install → app login → broker(s) authenticated (bridge + Dhan native path proven; other natives' flows implemented and mock-verified) → next-day restart → sessions refresh or cleanly re-prompt. Practice mode usable end-to-end (JWT-correct). Multi-agent audit + re-audit clean.
