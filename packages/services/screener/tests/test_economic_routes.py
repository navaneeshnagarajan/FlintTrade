"""Tests for packages/services/screener/src/economic_routes.py — economic calendar endpoint."""
...
import flinttrade_screener.economic_routes as mod
...
def test_calendar_ok(client):
    ...
    resp = client.get("/v1/economic/calendar")
...
def test_calendar_with_days(client):
    ...
    resp = client.get("/v1/economic/calendar?days=7")
    assert resp.status_code == 200
    body = resp.get_json()
...
def test_calendar_with_country_filter(client):
    ...
    resp = client.get("/v1/economic/calendar?country=IN&impact=high")
    assert resp.status_code == 200


def test_calendar_invalid_days(client):
    """400 or default when days is invalid."""
    resp = client.get("/v1/economic/calendar?days=abc")
