...
class GatedEmergencyBrokerDispatcher:
    ...
    * ``run_awaitable`` marshals router work onto the owning broker event loop.
    ...
    def _dispatch_legacy_target(
        ...
        *,
        reason_hash: str,
        initial_router: EmergencyRouter | None = None,
    ) -> tuple[EmergencyVerbOutcome, ...]:
        outcomes: list[EmergencyVerbOutcome] = []
        selector = target.request_ctx.selector
        journal_source = self._journal_source(policy)
        try:
            unresolved = {
                (record.parent_verb, record.scope): record
                for record in self._intent_journal.unresolved(
                    selector,
                    policy.verbs,
                    source=journal_source,
                )
        ...
        for index, verb in enumerate(policy.verbs):
            try:
                router = initial_router if index == 0 and initial_router is not None else self._router_provider()
            except Exception as exc:  # noqa: BLE001 - provider failure is bounded
                logger.error(
            ...
            write = EmergencyBrokerWrite(parent_verb=verb, verb=verb, payload=canonical)
            scope = self._intent_scope(write)
            if (verb, scope) in unresolved:
                outcomes.append(
                    EmergencyVerbOutcome(
                        verb,
                        succeeded=False,
                        attempted=False,
                        failure_code="dispatch_uncertain",
                        selector=selector,
                    )
                )
                continue
            try:
                intent_record, created = self._intent_journal.reserve(
                    ...
                    parent_verb=verb,
                    verb=verb,
                    payload_hash=_canonical_order_hash(canonical),
                    scope=scope,
                )
            except Exception as exc:  # noqa: BLE001 - reservation must precede dispatch
                logger.error("Emergency intent reservation failed (%s)", type(exc).__name__)
                outcomes.append(
                    EmergencyVerbOutcome(
                        verb,
                ...
                continue
            unresolved[(verb, scope)] = intent_record
            if not created:
                outcomes.append(
                    EmergencyVerbOutcome(
                        verb,
            ...
            try:
                ...
                broker_result = self._run_awaitable(
                    router.execute_gated(
                        request_ctx,
                        verb=verb,
                        payload=canonical,
                ...
                _mark_adapter_invoked()
                broker_failure = self._broker_result_failure(broker_result)
                if broker_failure:
                    self._intent_journal.mark_unknown(intent_record.intent_id)
                    outcomes.append(
                        EmergencyVerbOutcome(
                            verb,
                            succeeded=False,
                            failure_code=broker_failure,
                            selector=selector,
                        )
                    )
                    continue
                self._intent_journal.acknowledge(
                    intent_record.intent_id,
                    self._result_order_ids(broker_result),
                )
                ...
                if adapter_invoked:
                    ...
                    try:
                        ...
                        unresolved.pop((verb, scope), None)
                outcomes.append(
                    EmergencyVerbOutcome(
                        verb,
                        succeeded=False,
                        attempted=adapter_invoked,
                        failure_code=failure_code,
                        selector=selector,
                    )
                )
                continue
            outcomes.append(EmergencyVerbOutcome(verb, succeeded=True, selector=selector))
