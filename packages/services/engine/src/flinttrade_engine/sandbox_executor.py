...
Usage::
    ...
    from flinttrade_engine.sandbox_executor import SandboxExecutor
...
class SandboxExecutor:
    ...
    def _run_in_subprocess(
        ...
        2. Spawn ``python -I -S -m flinttrade_engine._sandbox_child``
        ...
        # both in the source checkout (where ``flinttrade_engine`` is
        ...
        # ``flinttrade_engine._sandbox_child`` dotted spec doesn't
        ...
        # ``from flinttrade_engine import sandbox_executor as se``
        ...
        popen_kwargs: dict[str, Any] = {
            ...
            "env": env,
            # SC-03: do not leak inherited file descriptors into the sandbox
            # child. close_fds=True is the 3.7+ default but stated explicitly
            # for the gate-#8 grep; pass_fds=() (POSIX) is the real FD-isolation
            # control — the child inherits only its three std streams.
            "close_fds": True,
        }
        if sys.platform != "win32":
