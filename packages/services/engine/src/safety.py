...
class SafetyBypassError(RuntimeError):
    """Raised when a write-side broker action lacks a valid safety context."""


@dataclass(frozen=True)
class SafetyContext:
    """Single-use signed proof that the safety gate approved one order."""
    ...
    @staticmethod
    def fingerprint_order(order: object) -> str:
        payload = repr(sorted(getattr(order, "__dict__", {}).items()))
    ...
    def mint(
        ...
        *,
        ...
        order_fingerprint = cls.fingerprint_order(order)
    ...
    def verify(
        ...
        *,
        ...
        if datetime.now(tz=timezone.utc).timestamp() > self.expires_at:
            return False
        if self.order_fingerprint != self.fingerprint_order(order):
            return False
        if (self.mode, self.jti, self.actor_type, self.actor_id) != (mode, jti, actor_type, actor_id):
            return False
        if (self.intent_source, self.external_nonce_hash) != (intent_source, external_nonce_hash):
            return False
    ...
    @staticmethod
    def _material(*parts: object) -> str:
