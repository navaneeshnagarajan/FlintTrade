"""Morning Star candlestick strategy — Pattern category.

The Morning Star is a 3-candle bullish reversal pattern:
    Bar -2 (first):  Large bearish candle  (close < open, body > body_min_pct of range)
    Bar -1 (middle): Small body star       (|close - open| <= star_body_max * range)
                     — the star gaps down from bar -2 (close < bar -2 close)
    Bar  0 (current): Large bullish candle (close > open, body > body_min_pct of range)
                     — closes above the midpoint of bar -2's body (deep penetration)

An optional SMA trend filter ensures we only take the pattern in a downtrend
(price below SMA), increasing the probability of a genuine reversal.

Exit is time-based after hold_bars candles or when price crosses back below
the open of the current bar (protective stop).
"""
...
logger = logging.getLogger("flinttrade.backtest.strategies.pattern_morning_star")


class MorningStar(BaseStrategy, _BacktestStrategyMixin):
    """Morning Star 3-candle bullish reversal strategy.

    Entry: BUY when a complete Morning Star pattern is detected.
    Exit: Time-based after hold_bars bars or price back below entry.

    Args:
        sma_period: SMA period for downtrend context filter (default 20).
    ...
    def on_bar(self, bar: OHLCV) -> None:
        """Process a completed bar and detect Morning Star patterns.
        ...
        c1_bearish = c1 < o1 and br1 >= self.body_min_pct

        # --- Candle 2: small star body ---
        br2 = self._body_ratio(o2, c2, h2, lo2)
        # Star must gap down: its high is below or at candle 1's close
        c2_star = br2 <= self.star_body_max and max(o2, c2) <= c1

        # --- Candle 3: large bullish, deep penetration into candle 1 ---
        br3 = self._body_ratio(o3, c3, h3, lo3)
        midpoint_c1 = o1 - (o1 - c1) * self.penetration  # midpoint of c1's body
        c3_bullish = c3 > o3 and br3 >= self.body_min_pct and c3 >= midpoint_c1

        # Optional trend filter: pattern should occur in a downtrend
        trend_ok = True
        if self.sma_period > 0:
            sma_vals = sma(self._closes, self.sma_period)
            trend_ok = c3 <= sma_vals[-1] or sma_vals[-1] == 0

        if c1_bearish and c2_star and c3_bullish and trend_ok:
            logger.debug(
                "Morning Star | c1_close=%.2f c2_body=%.4f c3_close=%.2f | %s",
                c1,
                br2,
                c3,
                self._symbol,
