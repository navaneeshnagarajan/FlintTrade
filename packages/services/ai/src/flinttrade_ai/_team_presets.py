"""Private canonical catalogue for FlintTrade multi-agent team presets."""
...
_ROLE_TYPES = {
    ...
    "risk_manager": AgentRoleType.RISK_MANAGER,
    "pairs_researcher": AgentRoleType.TECHNICAL,
    "cointegration_tester": AgentRoleType.TECHNICAL,
    "stat_arb_executor": AgentRoleType.AGGREGATOR,
    "feature_engineer": AgentRoleType.TECHNICAL,
    "model_trainer": AgentRoleType.TECHNICAL,
    "signal_generator": AgentRoleType.AGGREGATOR,
    "macro_analyst": AgentRoleType.FUNDAMENTAL,
    "rates_analyst": AgentRoleType.FUNDAMENTAL,
    "fx_analyst": AgentRoleType.AGGREGATOR,
    "earnings_analyst": AgentRoleType.FUNDAMENTAL,
    "news_scanner": AgentRoleType.SENTIMENT,
    "sentiment_scorer": AgentRoleType.AGGREGATOR,
    "sector_ranker": AgentRoleType.TECHNICAL,
    "momentum_calculator": AgentRoleType.TECHNICAL,
    "sector_rebalancer": AgentRoleType.AGGREGATOR,
    "tape_reader": AgentRoleType.TECHNICAL,
    "level2_analyst": AgentRoleType.TECHNICAL,
    "scalp_executor": AgentRoleType.AGGREGATOR,
    "fundamental_analyst": AgentRoleType.FUNDAMENTAL,
    "valuation_model": AgentRoleType.FUNDAMENTAL,
    "portfolio_optimizer": AgentRoleType.AGGREGATOR,
    "var_calculator": AgentRoleType.RISK_MANAGER,
    "stress_tester": AgentRoleType.RISK_MANAGER,
    "order_safety_checker": AgentRoleType.RISK_MANAGER,
    "derivatives_lead": AgentRoleType.TECHNICAL,
    "stat_arb_lead": AgentRoleType.TECHNICAL,
    "ml_quant_lead": AgentRoleType.TECHNICAL,
    "macro_lead": AgentRoleType.FUNDAMENTAL,
    "event_lead": AgentRoleType.SENTIMENT,
    "sector_rotation_lead": AgentRoleType.TECHNICAL,
    "scalp_lead": AgentRoleType.TECHNICAL,
    "portfolio_research_lead": AgentRoleType.FUNDAMENTAL,
    "risk_committee_lead": AgentRoleType.RISK_MANAGER,
    "chief_investment_officer": AgentRoleType.AGGREGATOR,
}
...
class TeamPreset:
    ...
    Attributes:
        ...
        agents: Ordered list of ``TeamPresetAgent`` objects.
    """

    name: str
    description: str
    agents: list[TeamPresetAgent] = field(default_factory=list)
    ...
    def to_agent_roles(self) -> list[AgentRole]:
        """Convert the preset definitions to roles consumed by ``AgentTeam``."""
        return [
            AgentRole(
                name=agent.role.replace("_", " ").title(),
                role_id=agent.role,
                role_type=_ROLE_TYPES[agent.role],
...
_PRESET_REGISTRY: dict[str, TeamPreset] = {}
...
def get_preset(name: str) -> TeamPreset:
    ...
    Returns:
        The corresponding ``TeamPreset`` instance.
    ...
    try:
        ...
        raise KeyError(f"Unknown swarm preset '{name}'. Available presets: {available}") from None


def get_all_presets() -> list[TeamPreset]:
    """Return all registered ``TeamPreset`` objects.

    Returns:
        List of ``TeamPreset`` instances in alphabetical name order.
    """
