...
#  only activates when pytest is invoked from packages/gateway/.  When run
#  from the repo root we must inject it manually.)
# ---------------------------------------------------------------------------
_REPO_ROOT = Path(__file__).resolve().parents[3]
_GATEWAY_SRC = str(_REPO_ROOT / "packages" / "gateway" / "src")
...
def test_reconnect_with_no_saved_accounts(caplog: pytest.LogCaptureFixture) -> None:
    ...
    from packages.core.src.app import _reconnect_saved_accounts  # noqa: PLC0415
...
def test_reconnect_partial_failure(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with patch("packages.gateway.src.session.BrokerSession", side_effect=_make_session):
