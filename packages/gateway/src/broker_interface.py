...
``packages.core.src.openalgo_client``.  Broker-specific adapters (future) can
implement the same Protocol independently.

Usage::

    from packages.gateway.src.broker_interface import BrokerRegistry, OpenAlgoBroker
...
class OpenAlgoBroker:
    ...
    Delegates all broker operations to ``packages.core.src.openalgo_client``
