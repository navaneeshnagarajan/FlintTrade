...
class TestQuestDBBridgeHealth:
    ...
    def test_health_returns_true_when_questdb_responds_200(self):
        from packages.data.src.questdb_bridge import QuestDBBridge
...
class TestQuestDBBridgeInsertTicks:
    ...
    def test_insert_ticks_returns_count_of_valid_ticks(self):
        ...
        with patch("packages.data.src.questdb_bridge.socket.create_connection", return_value=mock_sock):
    ...
    def test_insert_ticks_empty_list_returns_zero(self):
        ...
        bridge = QuestDBBridge()
        with patch("packages.data.src.questdb_bridge.socket.create_connection") as mock_conn:
    ...
    def test_insert_ticks_raises_on_tcp_connection_failure(self):
        from packages.data.src.questdb_bridge import QuestDBBridge, QuestDBBridgeError

        ticks = [{"symbol": "NIFTY", "exchange": "NFO", "ltp": 22450.0}]
        bridge = QuestDBBridge()

        with patch(
            "packages.data.src.questdb_bridge.socket.create_connection",
