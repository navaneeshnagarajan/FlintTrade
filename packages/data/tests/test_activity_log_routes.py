...
Run with:
    python -m pytest packages/data/tests/test_activity_log_routes.py -v --import-mode=importlib
"""
...
class TestAuditLog:
    ...
    def test_returns_200(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log")
    ...
    def test_total_matches_seeded_entries(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log", per_page=500)
    ...
    def test_pagination_per_page(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log", per_page=3)
        data = resp.get_json()["data"]
        assert len(data["entries"]) == 3
        assert data["per_page"] == 3

    def test_pagination_page_2(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log", per_page=3, page=2)
        data = resp.get_json()["data"]
        assert len(data["entries"]) <= 3
        assert data["page"] == 2
    ...
    def test_filter_by_action(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log", action="order.place", per_page=500)
        data = resp.get_json()["data"]
        assert data["total"] == 2
        assert all(e["action"] == "order.place" for e in data["entries"])

    def test_filter_by_user(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log", user="bob", per_page=500)
        data = resp.get_json()["data"]
        assert data["total"] == 1
        assert data["entries"][0]["user"] == "bob"

    def test_filter_by_user_admin(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log", user="admin", per_page=500)
        data = resp.get_json()["data"]
        assert data["total"] == 2

    def test_until_filter_excludes_later_entries(self, route_client):
        # until a far-past timestamp → 0 results
        resp = _get(route_client, "/ft-api/v1/audit/log", until="2000-01-01T00:00:00+05:30", per_page=500)
        data = resp.get_json()["data"]
        assert data["total"] == 0

    def test_until_filter_includes_earlier_entries(self, route_client):
        # until a far-future timestamp → all results
        resp = _get(route_client, "/ft-api/v1/audit/log", until="2099-12-31T23:59:59+05:30", per_page=500)
        data = resp.get_json()["data"]
        assert data["total"] == len(_ACTIONS)

    def test_invalid_page_returns_400(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log", page="notanint")
        assert resp.status_code == 400

    def test_invalid_per_page_returns_400(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log", per_page="notanint")
        assert resp.status_code == 400
    ...
    def test_page_beyond_range_returns_empty_entries(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log", page=999, per_page=50)
        data = resp.get_json()["data"]
        assert data["entries"] == []

    def test_per_page_clamped_to_500(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/log", per_page=9999)
        data = resp.get_json()["data"]
...
class TestAuditExport:
    ...
    def test_returns_200(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/export")
        assert resp.status_code == 200
    ...
    def test_csv_filter_by_action(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/export", action="auth.login")
        reader = csv.reader(io.StringIO(resp.data.decode("utf-8")))
    ...
    def test_csv_filter_by_user(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/export", user="system")
        reader = csv.reader(io.StringIO(resp.data.decode("utf-8")))
        rows = list(reader)[1:]
        assert len(rows) == 2
    ...
    def test_csv_until_far_past_returns_only_header(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/export", until="2000-01-01T00:00:00+05:30")
        reader = csv.reader(io.StringIO(resp.data.decode("utf-8")))
        rows = list(reader)
        # Only header row
        assert len(rows) == 1
...
class TestAuditStats:
    ...
    def test_returns_200(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/stats")
        assert resp.status_code == 200
    ...
    def test_filter_by_user(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/stats", user="alice")
        data = resp.get_json()["data"]
    ...
    def test_filter_by_since_far_future_returns_zero(self, route_client):
        resp = _get(route_client, "/ft-api/v1/audit/stats", since="2099-01-01T00:00:00+05:30")
        data = resp.get_json()["data"]
        assert data["total"] == 0
