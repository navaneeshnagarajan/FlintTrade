...
def _make_agg(time_bin_seconds: int = 300, tick_size: float = 50.0):
    from packages.data.src.orderflow_aggregator import OrderFlowAggregator
...
class TestCumulativeDelta:
    ...
    def test_single_bucket_equals_its_delta(self):
        from packages.data.src.orderflow_aggregator import FootprintBucket, OrderFlowAggregator
...
class TestFootprintBucketDataclass:
    def test_total_volume_is_buy_plus_sell(self):
        from packages.data.src.orderflow_aggregator import FootprintBucket
