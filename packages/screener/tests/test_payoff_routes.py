"""Tests for packages/screener/src/payoff_routes.py — payoff, regime, correlation endpoints."""
...
import packages.screener.src.payoff_routes as mod
...
def test_payoff_analyse_no_legs(client):
    ...
    resp = client.post("/ft-api/v1/payoff/analyse", json={"legs": [], "spot": 24000.0})
...
def test_payoff_curve_no_legs(client):
    ...
    resp = client.post("/ft-api/v1/payoff/curve", json={"spot": 24000.0})
    assert resp.status_code == 400
...
def test_regime_current_ok(client):
    ...
    with patch.object(mod._regime_detector, "detect", return_value=signal):
        resp = client.get("/ft-api/v1/regime/current?vix=15.0")
    assert resp.status_code == 200
    body = resp.get_json()
    assert body["status"] == "success"
...
def test_regime_current_defaults(client):
    ...
    signal = _make_regime_signal()
    with patch.object(mod._regime_detector, "detect", return_value=signal):
        resp = client.get("/ft-api/v1/regime/current")
    assert resp.status_code == 200
...
def test_regime_current_detector_error(client):
    ...
    with patch.object(
        ...
        resp = client.get("/ft-api/v1/regime/current?vix=20.0")
    assert resp.status_code == 500
    body = resp.get_json()
    assert body["status"] == "error"
...
def test_correlation_post_ok(client):
    ...
    with patch.object(mod._correlation_engine, "compute", return_value=result), patch(
        ...
        resp = client.post(
            "/ft-api/v1/analytics/correlation",
...
def test_correlation_get_ok(client):
    ...
    with patch.object(mod._correlation_engine, "compute", return_value=result), patch(
        ...
        resp = client.get("/ft-api/v1/analytics/correlation?vix=15.0")
    assert resp.status_code == 200
    body = resp.get_json()
    assert body["status"] == "success"
...
def test_correlation_sample_data_flag(client):
    ...
    with patch.object(mod._correlation_engine, "compute", return_value=result), patch(
        ...
        resp = client.post("/ft-api/v1/analytics/correlation", json={})
    assert resp.status_code == 200
    body = resp.get_json()
