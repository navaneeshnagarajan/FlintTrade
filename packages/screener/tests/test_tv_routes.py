"""Tests for packages/screener/src/tv_routes.py — TradingView signals endpoints."""
...
import packages.screener.src.tv_routes as mod
...
def test_tv_analysis_ok(client):
    ...
    with patch("packages.screener.src.tv_routes.tv_signals", create=True), patch(
        "packages.screener.src.tv_signals.TVSignals", return_value=mock_tv
    ):
        # Patch the lazy import inside the handler
        with patch(
            "packages.screener.src.tv_routes.tv_bp.view_functions",
            wraps=mod.tv_bp.view_functions,
        ):
            with patch(
                "packages.screener.src.tv_signals.TVSignals"
            ) as cls_mock:
...
def test_tv_analysis_upstream_error(client):
    ...
    with patch("packages.screener.src.tv_signals.TVSignals", return_value=mock_tv):
