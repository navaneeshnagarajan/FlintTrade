...
def _inf(tick_size: float = 0.05, bucket_sec: int = 60):
    from packages.screener.src.orderflow_inference import OrderFlowInference
...
class TestPriceLevel:
    def test_delta_buy_heavy(self):
        from packages.screener.src.orderflow_inference import PriceLevel
...
class TestFlowBucket:
    def test_dominant_side_buy(self):
        from packages.screener.src.orderflow_inference import FlowBucket
...
class TestRoundToTick:
    def test_exact_multiple(self):
        from packages.screener.src.orderflow_inference import _round_to_tick
