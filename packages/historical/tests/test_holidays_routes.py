...
Run with:
    python -m pytest packages/historical/tests/test_holidays_routes.py -v --import-mode=importlib
"""
...
class TestHolidaysRoute:
    ...
    def test_default_params_returns_200(self, client) -> None:  # type: ignore[no-untyped-def]
        ...
        with (
            patch("packages.historical.src.holidays_routes.OpenAlgoClient", return_value=mock),
            patch("packages.historical.src.holidays_routes.Settings"),
        ):
    ...
    def test_openalgo_failure_returns_fallback(self, client) -> None:  # type: ignore[no-untyped-def]
        ...
        with (
            patch(
                "packages.historical.src.holidays_routes.OpenAlgoClient",
