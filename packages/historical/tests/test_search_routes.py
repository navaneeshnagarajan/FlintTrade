...
Run with:
    python -m pytest packages/historical/tests/test_search_routes.py -v --import-mode=importlib
"""
...
from packages.historical.src.search_routes import search_bp
...
class TestSearchRoute:
    ...
    def test_basic_search_returns_results(self, client) -> None:  # type: ignore[no-untyped-def]
        ...
        with (
            patch("packages.historical.src.search_routes.OpenAlgoClient", return_value=mock),
            patch("packages.historical.src.search_routes.Settings"),
        ):
