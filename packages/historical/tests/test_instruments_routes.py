...
Run with:
    python -m pytest packages/historical/tests/test_instruments_routes.py -v --import-mode=importlib
"""
...
from packages.historical.src.instruments_routes import instruments_bp, _STREAM_THRESHOLD
...
class TestInstrumentsRoute:
    ...
    def test_default_exchange_returns_200(self, client) -> None:  # type: ignore[no-untyped-def]
        ...
        with (
            patch("packages.historical.src.instruments_routes.OpenAlgoClient", return_value=mock),
            patch("packages.historical.src.instruments_routes.Settings"),
        ):
    ...
    def test_openalgo_failure_returns_503(self, client) -> None:  # type: ignore[no-untyped-def]
        ...
        with (
            patch(
                "packages.historical.src.instruments_routes.OpenAlgoClient",
