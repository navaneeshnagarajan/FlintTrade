...
Config keys expected on ``app.config``:
    ``BASKET_EXECUTOR`` — a :class:`~packages.engine.src.basket_orders.BasketOrderExecutor`
    ``SPLIT_EXECUTOR``  — a :class:`~packages.engine.src.split_orders.SplitOrderExecutor`
...
def place_options_strategy() -> Response:
    ...
    from packages.engine.src.options_multi_order import OptionsStrategyBuilder
...
def _build_strategy_legs(
    ...
    Returns:
        List of :class:`~packages.engine.src.basket_orders.BasketLeg`.
