...
Run with:
    python -m pytest packages/engine/tests/test_leverage_routes.py -v --import-mode=importlib
"""
...
from packages.engine.src.leverage_routes import leverage_bp
...
class TestLeverageMarginRoute:
    ...
    def test_returns_200_on_success(self, client) -> None:  # type: ignore[no-untyped-def]
        ...
        with (
            patch("packages.engine.src.leverage_routes.OpenAlgoClient", return_value=_mock_client(margin_payload)),
            patch("packages.engine.src.leverage_routes.Settings"),
        ):
    ...
    def test_openalgo_failure_returns_503(self, client) -> None:  # type: ignore[no-untyped-def]
        ...
        with (
            patch(
                "packages.engine.src.leverage_routes.OpenAlgoClient",
