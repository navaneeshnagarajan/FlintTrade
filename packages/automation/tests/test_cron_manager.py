...
class TestJobDefinitionDataclass:
    def test_default_status_is_active(self):
        from packages.automation.src.cron_manager import JobDefinition, JobStatus
        job = JobDefinition(name="foo")
        assert job.status == JobStatus.ACTIVE.value

    def test_history_starts_empty(self):
        from packages.automation.src.cron_manager import JobDefinition
        job = JobDefinition(name="foo")
        assert job.history == []
    ...
    def test_job_history_fields(self):
        from packages.automation.src.cron_manager import JobHistory
        h = JobHistory(
            job_name="test",
...
class TestJobStatusEnum:
    def test_enum_values(self):
        from packages.automation.src.cron_manager import JobStatus
...
class TestIsMarketHoliday:
    def test_saturday_is_holiday(self):
        from packages.automation.src.cron_manager import _is_market_holiday
        # Patch datetime.now to return a Saturday
        saturday = datetime(2026, 3, 14, 10, 0, tzinfo=IST)
        with patch("packages.automation.src.cron_manager.datetime") as mock_dt:
...
class TestLoadHolidays:
    def test_loads_holidays_list(self):
        import asyncio
        from packages.automation.src.cron_manager import load_holidays_from_client
        mock_client = MagicMock()
