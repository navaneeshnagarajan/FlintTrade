...
describe('homepage Graphite Continuity A1 (bands + CTA + motion)', () => {
  ...
  it('mobile (max-width: 620px) makes essential hero content immediately useful with exact child selectors (no opacity:0 or multi-second delay on title/slogan/actions; includes .hero-title-char)', () => {
    expect(cssSource).toMatch(/@media \(max-width:\s*620px\)/);
    // Immediate visibility overrides for hero essentials on small viewports — exact child selectors required
    expect(cssSource).toMatch(/@media \(max-width:\s*620px\)[\s\S]{0,400}?\.hero-title-char/);
    expect(cssSource).toMatch(/@media \(max-width:\s*620px\)[\s\S]{0,400}?\.hero-copy h1,\s*\.hero-title-char/);
    expect(cssSource).toMatch(/@media \(max-width:\s*620px\)[\s\S]{0,400}?\.hero-slogan span/);
    // Immediate visibility overrides for hero essentials on small viewports
    expect(cssSource).toMatch(/max-width:\s*620px[\\s\\S]{0,300}?(hero-copy|hero-title|hero-slogan|hero-actions).*opacity:\\s*1|opacity:\\s*1[\\s\\S]{0,100}?(hero-copy|hero-title|hero-slogan|hero-actions)/);
  });
