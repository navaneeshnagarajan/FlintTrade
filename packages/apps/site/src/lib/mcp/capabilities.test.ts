...
describe('docs index generation', () => {
  ...
  it('publishes the current terminal widget catalogue count', () => {
    ...
    expect(architectureDoc?.content).toContain('97 widgets');
    expect(architectureDoc?.content).toContain('26 trading + 44 analysis + 27 utility');
    expect(userGuideDoc?.content).toContain('The widgets (97)');
  ...
  it('keeps homepage facts aligned with the audited gateway and widget state', () => {
    ...
    expect(pageSource).toContain('<strong>97</strong>');
    expect(pageSource).not.toContain('<strong>98</strong>');
