...
// No React, no chart API — these are plain math utilities.
...
export function calcPivotPoints(bars: OhlcvBar[]): PivotResult | null {
...
export function calcVWAP(bars: OhlcvBar[], times: Time[]): (number | null)[] {
...
export function calcVWMA(bars: OhlcvBar[], period = 20): (number | null)[] {
  const n = bars.length;
  const result: (number | null)[] = new Array(n).fill(null);
  for (let i = period - 1; i < n; i++) {
    let sumCV = 0;
