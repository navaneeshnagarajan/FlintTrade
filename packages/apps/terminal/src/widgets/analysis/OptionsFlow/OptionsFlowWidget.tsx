  ...
  return (
    ...
    >
      ...
      <div className="flex-none flex items-center gap-2 px-2 py-1.5 bg-surface-card border-b border-border-default">
        ...
        {/* Honest disclosure — the widget renders SAMPLE_FLOW unconditionally
            because no backend `/api/v1/analysis/options-flow` endpoint exists
            yet. The badge MUST stay visible even when a broker is connected:
            connecting a broker does not make this hardcoded flow live. When that
            endpoint lands, gate this on `isConnected && mode !== "live"` and add
            a TanStack Query call. */}
        <span
          className="inline-flex items-center rounded border border-amber-500/40 bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-medium text-amber-400"
          role="status"
          aria-label="Showing sample data; no live options-flow source is wired yet"
          title="No live data wired yet — showing sample options flow so the widget is usable in explore mode."
        >
          Sample data
        </span>
