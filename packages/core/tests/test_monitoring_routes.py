...
def app_client(monkeypatch_module):
    ...
    traffic.record("GET", "/ft-api/v1/health", 200, 12.0)
    traffic.record("POST", "/ft-api/v1/pnl-tracker", 201, 5.0)
    traffic.record("GET", "/ft-api/v1/bad", 500, 3.0)
...
class TestMonitoringRoutes:
    ...
    def test_health_returns_200_or_503(self, app_client):
        resp = _get(app_client, "/ft-api/v1/health")
    ...
    def test_traffic_stats_returns_data(self, app_client):
        resp = _get(app_client, "/ft-api/v1/traffic/stats")
    ...
    def test_traffic_recent_returns_list(self, app_client):
        resp = _get(app_client, "/ft-api/v1/traffic/recent")
        assert resp.status_code == 200
        data = json.loads(resp.data)
        assert data["status"] == "ok"
    ...
    def test_latency_stats_has_broker(self, app_client):
        resp = _get(app_client, "/ft-api/v1/latency/stats")
        assert resp.status_code == 200
        data = json.loads(resp.data)
        assert data["status"] == "ok"
    ...
    def test_latency_recent_returns_list(self, app_client):
        resp = _get(app_client, "/ft-api/v1/latency/recent")
        assert resp.status_code == 200
        data = json.loads(resp.data)
        assert data["status"] == "ok"
        assert isinstance(data["data"], list)
