"""Tests for packages/core/data/src/audit_export_routes.py (Flask Blueprint).
...
def client(audit_logger: MagicMock, tmp_path):
    ...
    Yields:
        Flask test client.
    """
    from flinttrade_data.audit_export_routes import create_audit_export_blueprint
...
class TestAuditExportCsv:
    def test_csv_export_success(self, client: MagicMock, tmp_path) -> None:
        ...
        with patch(
            "flinttrade_data.audit_export.AuditExporter.to_csv",
...
class TestAuditSummary:
    def test_summary_success(self, client: MagicMock) -> None:
        ...
        with patch(
            "flinttrade_data.audit_export.AuditExporter.summary_stats",
            return_value=stats,
        ):
            resp = client.get(
