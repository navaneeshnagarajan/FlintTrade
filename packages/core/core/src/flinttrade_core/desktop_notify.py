"""Backend → desktop-shell native notification bridge.

The Tauri desktop shell drains the backend sidecar's stdout and raises a native
OS notification for each ``FLINTTRADE_NOTIFY\\t<title>\\t<body>`` line (see
``packages/apps/desktop/src-tauri/src/lib.rs``). This is the *producer* side:
best-effort, dependency-free, and a no-op unless the backend is actually running
under the desktop shell (``FLINTTRADE_DESKTOP=1``, set by the Rust sidecar
spawn) — so ``make start`` / CLI runs never print stray sentinel lines.
...
#: Stdout prefix the desktop shell parses. Must match ``NOTIFY_SENTINEL`` in
#: the Tauri shell (``lib.rs``). Tab-delimited: PREFIX \t title \t body.
NOTIFY_SENTINEL = "FLINTTRADE_NOTIFY"


def desktop_shell_active() -> bool:
    """Whether the backend is running under the Tauri desktop shell."""
