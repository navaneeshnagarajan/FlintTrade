"""Tests for packages/core/core/src/user_routes.py (Flask Blueprint).

Mocks the UserManager and JWT secret so tests run standalone.

Run with:
    python -m pytest packages/core/core/tests/test_user_routes.py -v --import-mode=importlib
"""
...
from flinttrade_core.user_routes import users_bp
...
@pytest.fixture()
def client(app):
    with patch("flinttrade_core.user_routes._get_jwt_secret", return_value=_JWT_SECRET), \
         patch("flinttrade_core.user_routes._is_multi_user_enabled", return_value=True):
...
def client_single_user(app):
    ...
    with patch("flinttrade_core.user_routes._get_jwt_secret", return_value=_JWT_SECRET), \
         patch("flinttrade_core.user_routes._is_multi_user_enabled", return_value=False):
        with app.test_client() as c:
            yield c
