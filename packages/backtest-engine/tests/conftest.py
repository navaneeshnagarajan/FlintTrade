...
Paths added (in priority order — highest priority last via insert(0)):
  1. repo root          → ``from packages.core.src.models import …`` etc.
  2. packages/core/src  → ``from models import OHLCV``
  3. packages/engine/src→ ``from strategy import BaseStrategy``
...
# Absolute path to this conftest (packages/backtest-engine/tests/)
_tests_dir = os.path.dirname(os.path.abspath(__file__))
_pkg_dir = os.path.join(_tests_dir, "..")        # packages/backtest-engine/
