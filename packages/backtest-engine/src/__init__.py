...
# Guard: when src/ is on sys.path via sys.path.insert (old test_backtest_engine.py),
# __package__ is None/empty. Skip all auto-imports to avoid relative import errors.
# The old test imports modules directly (from simulator import ...) so it doesn't
# need __init__.py to re-export anything.
if not __package__:
    __all__: list[str] = []
else:
