 ...
 *
 * Features:
 *   - ±20 price ticks from current price
...
export function generateLadderLevels(centerPrice: number, tickSize: number): LadderLevel[] {
  ...
  for (let i = LEVELS_ABOVE; i >= -LEVELS_BELOW; i--) {
    ...
    levels.push({
      price,
      bidQty: i < 0 ? Math.round(Math.random() * 800 + 100) : 0,
      askQty: i > 0 ? Math.round(Math.random() * 800 + 100) : 0,
    });
  }
  return levels;
}
...
function LevelRow({ level, isCenter, order, maxQty, qty, onBid, onAsk, onCancel }: {
  level: LadderLevel; isCenter: boolean; order?: PendingOrder;
  maxQty: number; qty: number;
...
function OrderLadderWidget({ symbol = "NIFTY", exchange = "NSE" }: Props) {
  ...
  const levels  = useMemo(() => generateLadderLevels(SAMPLE_PRICE, tickSize), [tickSize]);
  ...
  }, [mode, qty, symbol, exchange, track, showMsg]);
