...
import { Plus, X, RefreshCw, AlertCircle, Target } from "lucide-react";
...
// ---------------------------------------------------------------------------
// Canvas drawing (inline — mirrors RRGCanvas but uses portfolio symbols)
// ---------------------------------------------------------------------------
...
const PortfolioCanvas = memo(function PortfolioCanvas({ data }: PortfolioCanvasProps) {
  ...
  const toCanvasX = useCallback((rsRatio: number, canvasW: number, pad: number): number =>
    pad + ((rsRatio - AXIS_MIN) / (AXIS_MAX - AXIS_MIN)) * (canvasW - 2 * pad),
    [],
  );

  const toCanvasY = useCallback((rsMomentum: number, canvasH: number, pad: number): number =>
    (canvasH - pad) - ((rsMomentum - AXIS_MIN) / (AXIS_MAX - AXIS_MIN)) * (canvasH - 2 * pad),
    [],
  );
  ...
  }, [data, hoveredSymbol, toCanvasX, toCanvasY]);
