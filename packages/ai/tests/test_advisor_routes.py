"""Tests for packages/ai/src/advisor_routes.py (Flask Blueprint).
...
def app():
    ...
    """
    from packages.ai.src.advisor_routes import advisor_bp
...
class TestAdvisorChat:
    def test_messages_array_happy_path(self, client) -> None:
        ...
        with (
            patch("packages.ai.src.advisor_routes._is_llm_configured", return_value=True),
            patch("packages.ai.src.advisor_routes.LLMClient", return_value=mock_llm),
        ):
            resp = client.post(
                "/api/v1/advisor",
    ...
    def test_llm_not_configured_returns_503(self, client) -> None:
        ...
        Args:
            client: Flask test client.
        """
        with patch("packages.ai.src.advisor_routes._is_llm_configured", return_value=False):
    ...
    def test_missing_message_and_messages_returns_400(self, client) -> None:
        ...
        Args:
            client: Flask test client.
        """
        with patch("packages.ai.src.advisor_routes._is_llm_configured", return_value=True):
...
class TestAdvisorStatus:
    def test_status_configured(self, client) -> None:
        ...
        with (
            patch("packages.ai.src.advisor_routes._is_llm_configured", return_value=True),
            patch("packages.ai.src.advisor_routes.LLMConfig") as mock_cls,
        ):
