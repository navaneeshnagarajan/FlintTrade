...
const manualDocs = [
  {
    ...
    sourcePath: 'apps/site/src/app/mcp/page.tsx',
  ...
  },
  {
    ...
    sourcePath: 'apps/site/src/app/contribute/page.tsx',
...
async function main() {
  ...
  await writeFileAtomic(
    ...
    JSON.stringify({
      title: 'Packages',
      pages: packages.map((pkg) => pkg.name),
    }, null, 2),
  );
  ...
  const commands = [
    ...
    { label: 'Run focused terminal build', command: 'cd packages/terminal && npm run build' },
    { label: 'Run terminal unit tests', command: 'cd packages/terminal && npm run test' },
    { label: 'Run site checks', command: 'cd apps/site && npm run typecheck && npm run test && npm run build' },
  ];
