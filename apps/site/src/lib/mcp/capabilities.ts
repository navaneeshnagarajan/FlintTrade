...
export function explainRepoPath(repoPath: string): string {
  ...
  if (cleanPath.startsWith('apps/site')) {
    return 'apps/site is the public Next.js + Fumadocs website and docs/contribution MCP app. Run its checks with cd apps/site && npm run typecheck && npm run test && npm run build.';
  }
...
export function recommendTests(changedPaths: string[]): TestRecommendation {
  ...
  for (const changedPath of paths) {
    if (changedPath.startsWith('apps/site/')) {
      reasons.add('Public website, generated docs, or docs MCP changed.');
      commands.add('cd apps/site && npm run typecheck');
      commands.add('cd apps/site && npm run test');
      commands.add('cd apps/site && npm run build');
    } else if (changedPath.startsWith('packages/terminal/')) {
      reasons.add('React terminal code or tests changed.');
      commands.add('cd packages/terminal && npm run typecheck');
      commands.add('cd packages/terminal && npm run test');
      commands.add('cd packages/terminal && npm run build');
    } else if (changedPath.startsWith('packages/tick-engine/')) {
      reasons.add('Rust/PyO3 tick processing changed.');
      commands.add('cd packages/tick-engine && cargo test');
